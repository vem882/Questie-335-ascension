...
function QuestieSlash.HandleCommands(input)
    ...
    if mainCommand == "help" or mainCommand == "?" then
        ...
        if QuestieCompat.IsAscension then
            print(Questie:Colorize("/questie ascension [stats/npcs/quests/export] - " .. l10n("Ascension data collection commands"), "yellow"));
        end
        return;
    end
    ...
    -- /questie ascension [stats/npcs/quests/export]
    if mainCommand == "ascension" then
        if not QuestieCompat.IsAscension then
            print(Questie:Colorize("[Questie] ", "yellow") .. "Ascension commands are only available on Ascension WoW servers")
            return
        end
        
        local AscensionLoader = QuestieLoader:ImportModule("AscensionLoader")
        if AscensionLoader and AscensionLoader.ShowStats then
            AscensionLoader:ShowStats(subCommand or "stats")
        else
            print(Questie:Colorize("[Questie] ", "yellow") .. "Ascension module not loaded")
        end
        return
    end
