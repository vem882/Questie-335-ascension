...
local GitHubVersionCheck = _G.GitHubVersionCheck
---@type QuestieShutUp
...
QuestieInit.Stages[1] = function() -- run as a coroutine
    ...
    -- Initialize Ascension support if detected
    if QuestieCompat.IsAscension then
        local AscensionLoader = QuestieLoader:ImportModule("AscensionLoader")
        if AscensionLoader and AscensionLoader.Initialize then
            AscensionLoader:Initialize()
            coYield()
        end
    end
